...
- 全局快捷键录音，支持 `toggle` 和 `hold` 两种模式：
  ...
  - 两种模式下，停止缓冲（`stop_delay_ms`）期间再次按下热键都会取消停止、继续本次录音，不会把一句话拆成两次上屏。
...
- Wayland / X11 / macOS 顶层录音状态胶囊提示：录音时上方显示 `REC`（流式识别时显示最新识别文本），下方显示随声音起伏的蓝色波形，安静时自动放平；胶囊整体带淡蓝光晕，等待识别结果时波形缓缓收平。
- 同一时间只允许一个实例：daemon 与 TUI 共用一把 `flock` 锁，启动新 daemon 会自动接管并结束旧 daemon；已有 TUI 在运行时新实例会直接报错退出，避免两个实例同时录音导致文字重复上屏。
